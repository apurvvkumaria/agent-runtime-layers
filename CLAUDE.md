...
visible. It was built up in twenty-nine deliberate layers (see below), each adding one runtime
capability.
...
uv run python agent.py skills              # list registered skills + the tools each uses
uv run python agent.py marketplace         # list marketplace skills (+ installed status)
uv run python agent.py marketplace-install error_budget  # resolve deps, verify SHA256, install, load
uv run python agent.py skill-deps capacity_planner  # show a skill's dependency graph + install order
uv run python agent.py skill-reload        # report skills changed/added/removed since last load
uv run python agent.py skill-reload-demo   # edit a skill's source and hot-reload it live
...
| `skills/` | OpenClaw-pattern skills (Layer 23). Each `skills/<name>/` is a package with `SKILL.md` (capability declaration the LLM reads), `skill.py` (`async def <name>(arg, ctx=None)` — broker path via `ctx.call_tool` or direct fallback), and `policy.yaml` (skill-level capability policy). `context.py` defines `SkillContext` (`call_tool` + `call_skill` for composition); `registry.py` (`SkillRegistry`) auto-discovers them and wraps each as a LangChain tool. Each `SKILL.md` also carries `## Version` + `## Status` (Layer 25); the registry handles deprecation — warns on use and hides deprecated skills from `get_tools()`. Four skills: `research_and_summarize` (web search + metrics + LLM), `storage_health_check` (metrics + SLA-doc, rule-based), `cluster_briefing` (Layer 26 — composes the prior two via `ctx.call_skill`), and the deprecated `storage_sla_report` (delegates to `storage_health_check`). `marketplace.py` (`SkillMarketplace`, Layer 27) verifies + installs skills from a registry; `resolver.py` (`SkillResolver`, Layer 28) resolves a skill's `## Requires` into a deps-first install order; `SkillRegistry.reload()` + `reload_demo.py` (Layer 29) hot-reload edited/installed skills in-process. `get_tools()` includes the active ones; `agent skills` lists all with version/status, `agent skill "..."` runs research_and_summarize. |
| `marketplace/` | The skill marketplace / "remote" registry (Layer 27): `index.json` pins a SHA256 over each available skill package. Two installable skills: `error_budget/` (standalone) and `capacity_planner/` (which `## Requires` error_budget + storage_health_check — Layer 28 dependency resolution). `agent marketplace-install <name>` resolves deps, verifies each hash, then copies the packages into `skills/`. Source of truth lives here; installed copies under `skills/` are git-ignored. |
...
| `docs/` | Markdown read by the `filesystem` tool — `openShell_overview.md`, `sla_thresholds.md`, `agent_patterns.md`. The MCP filesystem server's only allowed directory. Also holds `nemoclaw_mapping.md` — a mapping of all 29 layers to their **NemoClaw** (OpenShell + OpenClaw + NeMo) equivalents (orientation doc, not a layer). |
...
| **23 — Skills as OpenClaw packages** | A skill is structurally identical to an OpenClaw skill | Each `skills/<name>/` is a package: `SKILL.md` (## What it does / When to use / Inputs / Outputs / Tools used internally / Example — the LLM reads this to route), `skill.py` (`async def <name>(arg, ctx=None)` — with a `ctx` every tool call is `await ctx.call_tool(...)` through the broker, exactly like OpenClaw's `ctx.call_claw`; `ctx=None` falls back to direct in-process calls), and `policy.yaml` (skill-level capability policy: egress / filesystem / resources). `skills/context.py` defines the `SkillContext` dataclass (`sandbox_id`, `policy`, async `call_tool`); `skills/registry.py` (`SkillRegistry`) `auto_discover()`s skills, parses each `SKILL.md`, and `as_tools()` / `inject_into_agent()` exposes them — so `get_tools()` carries them and the agent calls a skill by name like any tool. Two skills: `research_and_summarize` (web search + metrics + LLM) and `storage_health_check` (metrics + SLA doc, rule-based, no network). `agent skills` lists them. | Makes the OpenClaw bridge concrete: the same `SKILL.md` + `SkillContext` + `ctx.call_tool` shape, so a skill written here drops into OpenClaw (or runs under the OpenShell sandbox via the broker), with the SKILL.md-as-capability-declaration + per-skill policy being the OpenClaw contract. **Verified:** `agent ask "Research OpenShell and check if prod-us-east-1 is healthy"` auto-routes to *both* skills — `research_and_summarize` (→ web_search + storage_metrics + llm_summarize) and `storage_health_check` (→ storage_metrics + filesystem) — returning one combined report. |
| **24 — Skill-level evals** | A behavioral contract per skill | `evals/skill_evals.py` runs each skill in isolation (via `SkillRegistry`, the direct path) and asserts its output contract with deterministic deepeval metrics — `SubstringMetric` (all-of) + a custom `SubstringAnyMetric` (any-of). Cases: `research_and_summarize` must emit the three sections (`## Research Findings` / `## Storage Context` / `## Summary`); `storage_health_check` must name the cluster, cite `p99`, and emit a valid SLA rating (Excellent/Normal/Degraded/Critical) **and** a verdict (HEALTHY/NEEDS ATTENTION), regardless of the randomized metric values. Wired into `run_all` (a `SKILL EVALS` section + a `Skill evals: X/Y` summary line); runnable alone via `uv run python evals/skill_evals.py`. | Layer 9 evaluated the *agent* end-to-end; this evaluates each *skill* as its own unit — the skill is a contract boundary, so it gets its own contract test (structure/keywords the consumer relies on). Deterministic grading keeps them cheap and repeatable even though the skill runs for real. **Verified:** 2/2 — research emits all three sections; health correctly classifies (`Excellent` → `HEALTHY`). |
| **25 — Skill versioning & deprecation** | Skills carry a version and can be retired gracefully | Each `SKILL.md` declares `## Version` (semver) and `## Status` (active/deprecated); a deprecated skill adds a `## Deprecation` block (`Replaced by:` / `Remove after:`). `SkillRegistry` parses them and: tags deprecated skills `[DEPRECATED — use X]` in the tool description, prints a `[deprecated] … use Y; removed after <date>` warning (stderr) on invocation, and **excludes them from `as_tools()` by default** — so `get_tools()` never wires a deprecated skill into the agent — while keeping them resolvable via `get_skill()` for backward-compatible direct callers (`include_deprecated=True` re-enables them in `as_tools`). `agent skills` shows each skill's version + deprecation status. Demonstrated with `storage_sla_report` v0.9.0 (deprecated → `storage_health_check`, remove after 2026-09-30), whose `skill.py` delegates to its replacement. | Skills are an API surface, so they need API lifecycle: a version to pin against, and a deprecation path that keeps old callers working (warned, with a sunset date) while steering the agent and new code to the replacement — the same contract OpenClaw versioned skills carry. "Hidden from new wiring, still callable for compat" is the standard deprecation window. **Verified:** `agent skills` lists 3 skills (2 active); `get_tools()` exposes only the 2 active; invoking `storage_sla_report` warns and still returns the health report. |
| **26 — Skill composition (skills calling skills)** | A skill orchestrates *other skills*, not just tools | `SkillContext` gains `call_skill(name, arg)` (alongside `call_tool`): it resolves the sub-skill via `SkillRegistry.skill_function()` and runs it with a **depth-incremented child context**, so a chain of skills-calling-skills is bounded by `max_depth` (a runaway guard, like the heartbeat loop's). New composite skill `cluster_briefing` composes two existing skills — `storage_health_check` (live SLA assessment) + `research_and_summarize` (background) — into one briefing; with a `ctx` it uses `await ctx.call_skill(...)`, with `ctx=None` it calls the sub-skill functions directly. Its `policy.yaml` is the *union* of its parts' capabilities. | Layers 20/23 had a skill compose *tools*; this composes *skills* — a skill is itself a callable unit, so skills nest like functions. The depth guard is the runtime concern that matters: composition is a call graph that can recurse, so it needs a hard bound. **Verified:** `cluster_briefing("prod-us-east-1")` returns a `# Cluster Briefing` with a `## Health` section (p99 → rating, HEALTHY) *and* a `## Background` section (the full research report) — both sub-skills ran; the `max_depth` guard raises when exceeded. |
| **27 — Skill marketplace / remote loading** | Discover, verify, and install skills from a registry | `skills/marketplace.py` (`SkillMarketplace`): a registry (`marketplace/index.json`) lists skills and **pins a SHA256** over each package (SKILL.md + skill.py + policy.yaml); `install()` recomputes the hash and **refuses to materialize code on mismatch** (`IntegrityError`) before copying the verified package into `skills/`, where `SkillRegistry` then discovers it. The "remote" source is a local dir by default but configurable (`SKILL_MARKETPLACE` / `source=`), so an `http(s)://` registry slots in with the same fetch→verify→install contract. `agent marketplace` lists the catalog + installed status; `agent marketplace-install <name>` verifies + installs. Ships one skill, `error_budget` (pure arithmetic, no deps). The installed copy lands in `skills/error_budget/` (git-ignored; source of truth is `marketplace/`). | Remote skill loading is an arbitrary-code-execution + supply-chain surface, so it's integrity-first: trust is pinned by content hash and code is fail-closed (verify *before* install). Provenance (the hash) and containment (running the skill under the OpenShell sandbox, Layer 21) are complementary controls — verify *what* the code is, then bound *what it can do*. **Verified:** `marketplace-install error_budget` verifies + installs (then `agent skills` lists it and the agent can call it); a one-byte tamper makes install **refuse** with a checksum mismatch. |
| **28 — Skill dependency resolution** | Installing a skill pulls in its dependencies, in order | Skills declare `## Requires` in SKILL.md (other skills + optional semver constraints, e.g. `error_budget >= 1.0.0`). `skills/resolver.py` (`SkillResolver`) walks that graph for a marketplace target and returns the **install order** (deps first), partitioning each dep into *already satisfied* (installed + constraint met, via `SkillRegistry`) vs *to install* (in the marketplace + constraint met), and raising `DependencyError` on a missing/unsatisfiable dep or a **cycle**. `marketplace-install` now resolves first and installs each in order (every package still hash-verified, Layer 27); `agent skill-deps <name>` prints the graph + order. New marketplace skill `capacity_planner` requires `error_budget` (marketplace) + `storage_health_check` (core) — its direct-path import of `error_budget` only resolves *because* the resolver installs it first. Requires live in the hash-covered SKILL.md, so they can't be tampered undetected. | A skill registry is a package ecosystem, so it needs a package manager's job: a dependency graph, version constraints, a topological install order, and cycle detection. The interesting bit is the two satisfaction sources — already-installed core skills vs. fetch-from-marketplace — resolved together. **Verified:** `skill-deps capacity_planner` → order `error_budget → capacity_planner` with `storage_health_check` already satisfied; `marketplace-install capacity_planner` installs `error_budget` (dependency) then `capacity_planner`, each verified, and the composite then runs. |
| **29 — Skill hot-reloading** | Pick up edited / newly-installed skills without a restart | `importlib.import_module` returns the *cached* module, so a long-running process won't see an edited or marketplace-installed skill until it's `importlib.reload()`-ed. `SkillRegistry` now fingerprints each skill (SHA256 of its files) and stores the module; `reload()` re-scans, `importlib.reload()`s skills whose fingerprint changed, imports added ones (after `invalidate_caches()` so a new dir is found), drops removed ones, and returns the change-set — callers then re-fetch via `as_tools()`/`get_skill()`. `agent skill-reload` reports what changed on disk since the last load (persisted fingerprint snapshot); `agent skill-reload-demo` proves it in-process. | The concept is Python's module cache vs. live code: a running process's continuity means stale bytecode, so a hot path must deliberately re-exec changed modules. A real gotcha surfaced — `importlib.reload` trusts the `.pyc` `(mtime, size)`, so two same-second, same-size edits reuse stale bytecode (real edits/installs differ in size + span seconds, so it's a non-issue in practice). **Verified:** the demo skill returns `v1` → edit source → `reload()` → returns `v2`, no restart; and a marketplace install into a *live* registry shows up as `reload() added`. |
