...
visible. It was built up in twenty-five deliberate layers (see below), each adding one runtime
capability.
...
| `skills/` | OpenClaw-pattern skills (Layer 23). Each `skills/<name>/` is a package with `SKILL.md` (capability declaration the LLM reads), `skill.py` (`async def <name>(arg, ctx=None)` — broker path via `ctx.call_tool` or direct fallback), and `policy.yaml` (skill-level capability policy). `context.py` defines `SkillContext`; `registry.py` (`SkillRegistry`) auto-discovers them and wraps each as a LangChain tool. Each `SKILL.md` also carries `## Version` + `## Status` (Layer 25); the registry handles deprecation — warns on use and hides deprecated skills from `get_tools()`. Three skills: `research_and_summarize` (web search + metrics + LLM), `storage_health_check` (metrics + SLA-doc, rule-based), and the deprecated `storage_sla_report` (delegates to `storage_health_check`). `get_tools()` includes the active ones; `agent skills` lists all with version/status, `agent skill "..."` runs the first. |
