...
visible. It was built up in twenty-four deliberate layers (see below), each adding one runtime
capability.
...
| `skills/` | OpenClaw-pattern skills (Layer 23). Each `skills/<name>/` is a package with `SKILL.md` (capability declaration the LLM reads), `skill.py` (`async def <name>(arg, ctx=None)` — broker path via `ctx.call_tool` or direct fallback), and `policy.yaml` (skill-level capability policy). `context.py` defines `SkillContext`; `registry.py` (`SkillRegistry`) auto-discovers them and wraps each as a LangChain tool. Two skills: `research_and_summarize` (web search + metrics + LLM) and `storage_health_check` (metrics + SLA-doc, rule-based). `get_tools()` includes them; `agent skills` lists them, `agent skill "..."` runs the first. |
